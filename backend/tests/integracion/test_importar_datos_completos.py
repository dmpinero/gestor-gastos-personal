...
def _exportar(sesion_bd) -> bytes:
    return ExportarDatosCompletos(
        ...
        ListarAsociacionesDescripcion(RepositorioAsociacionesDescripcionSqlAlchemy(sesion_bd)),
        EscritorExportacionCompletaOpenpyxl(),
    ).ejecutar()
...
def test_importar_un_backup_sustituye_todos_los_datos_existentes(sesion_bd) -> None:
    ...
    assert resumen.asociaciones_descripcion_importadas == 1
    ...
    asociaciones_descripcion_finales = repo_asociaciones_descripcion.listar()
    assert len(asociaciones_descripcion_finales) == 1
    assert asociaciones_descripcion_finales[0].categoria_resumen_id == categoria_resumen_original.id
    assert asociaciones_descripcion_finales[0].descripcion == "Recibo Ayuntamiento"
